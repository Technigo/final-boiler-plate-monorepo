...
export const Smoke = forwardRef((props, ref) => {
  ...
  const splat = (x, y, dx, dy, color) => {
    ...
    gl.beginPath();
    gl.arc(x, y, config.SPLAT_RADIUS, 0, 2 * Math.PI, false);
    gl.fillStyle = `rgba(${color[0]}, ${color[1]}, ${color[2]}, 1)`;
    gl.fill();
  };

  useImperativeHandle(ref, () => ({
    addRandomSplats(numberOfSplats) {
      const canvas = canvasRef.current;
      if (!canvas) return;

      for (let i = 0; i < numberOfSplats; i++) {
        const color = [
          Math.random() * 10,
          Math.random() * 10,
          Math.random() * 10,
  ...
  useEffect(() => {
    ...
    // Replace 'window.onload = function () { m("h1"); };'
    // with the actual functionality you need on component load
