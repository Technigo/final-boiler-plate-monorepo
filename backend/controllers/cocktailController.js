...
// Store uploaded image directly on the server
const uploadImage = (req, res, next) => {
    if (!req.file) {
        return next();
    }

    const fileName = `<span class="math-inline">\{Date\.now\(\)\}</span>{path.extname(req.file.originalname)}`;
    const uploadPath = `uploads/${fileName}`;
    req.file.mv(uploadPath, async (err) => {
        if (err) {
            return res.status(500).json({ message: err.message });
        }

        req.body.imageUrl = uploadPath; // Add image URL to the request body
        next();
    });
};
